...
+function()
{
    ...
    let v = isPhone?1:2;
    function SPEED_X(){return v + RND() * v}
    function SPEED_Y(){return v + RND() * v}
    ...
    CreateBubble = function(ds)
    {
        for (let i = 0; i < ds.length; i++) {
            let e = ds[i]

            var bub = new Bubble(e);
            bub.setX(0);
            bub.setY(0);
            bub.vx = SPEED_X();
            bub.vy = SPEED_Y();
    ...
    function updateWall()
    {
        let d = 0;
        iLeft = d;
        iRight = de.clientWidth - D + d;

        iTop = d;
        iBottom = de.clientHeight - D + d;
    }
    ...
    /******************************
      * Class Bubble
      ******************************/
    function Bubble(e={title:'笑笑', url:'', datas:[],onclick:()=>{}})//{title}
    {
        var kOpa = [], kStp = [];
        var arrFlt = [];
        ...
        styBox.width = OD*scale + "px";
        styBox.height = OD*scale + "px";
        ...
        for(var i=0; i<4; i++)
        {
            ...
            sty.width = OD*scale+'px';
            sty.height = OD*scale+'px';
            // sty.transform = `scale(${scale})`
            ...
            {
                if(_IE6_)
                    sty.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src=../image/bubbles/heart.png)";
                else{
                    sty.backgroundImage = "url(../image/bubbles/heart.png)";
                    sty.backgroundSize = "100%";
                }
                break;
            }
            ...
            if(STD)
            {
                sty.backgroundImage = "url(../image/bubbles/ch" + i + ".png)";
                sty.backgroundSize = "100%";
                arrFlt[i] = sty;
            }
            else
            {
                sty.filter = "alpha progid:DXImageTransform.Microsoft.AlphaImageLoader(src=../image/bubbles/ch" + i + ".png)";
